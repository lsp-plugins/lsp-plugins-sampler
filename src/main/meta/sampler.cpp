...
#define LSP_PLUGINS_SAMPLER_VERSION_MICRO                   36
...
namespace lsp
{
    namespace meta
    {
        ...
        #define S_MIXER(id, sid)                      \
            SWITCH("ion_" #id, "Instrument on " #sid, "Inst on " #sid, 1.0f), \
            AMP_GAIN10("imix_" #id, "Instrument mix gain " #sid, "Inst gain" #sid, 1.0f), \
            PAN_CTL("panl_" #id, "Instrument panorama left " #sid, "Inst pan L " #sid, -100.0f), \
            PAN_CTL("panr_" #id, "Instrument manorama right " #sid, "Inst pan R " #sid, 100.0f), \
            BLINK("iact_" #id, "Instrument activity " #sid)

        #define S_DIRECT_OUT(id, sid)           \
            S_MIXER(id, sid),                        \
            SWITCH("don_" #id, "Direct output on " #sid, "DOut on " #sid, 1.0f), \
            AUDIO_OUTPUT("dol_" #id, "Direct output left " #sid), \
            AUDIO_OUTPUT("dor_" #id, "Direct output right " #sid)
